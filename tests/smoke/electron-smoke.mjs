...
const executablePath = process.env.THUNDERBOLT_FIGHTER_EXECUTABLE;
...
try {
  ...
  const ensureBossForSmokeCheck = (health = 1200) => window.evaluate((bossHealth) => {
    ...
    gameplay.enemies.find((enemy) => enemy.type === 'boss-class').health = bossHealth;
  ...
  await window.waitForFunction(() => {
    ...
    return Boolean(
      gameplay?.textures?.exists('player-ship')
      && gameplay?.textures?.exists('enemy-basic')
      && gameplay?.textures?.exists('gameplay-background')
      && gameplay?.cache?.audio?.exists('music-run')
      && gameplay?.cache?.audio?.exists('player-bolt-fire')
    );
  });
  ...
  if (executablePath) {
    ...
    await ensureBossForSmokeCheck(15);
    await window.evaluate(() => {
      const gameplay = globalThis.__thunderboltFighterGame.scene.getScene('gameplay');
      const boss = gameplay.enemies.find((enemy) => enemy.type === 'boss-class');
    ...
    const resultsLocalRecord = Number(await gameRoot.getAttribute('data-results-local-record'));
    const resultScore = Number(await gameRoot.getAttribute('data-results-score'));
    assert.equal(resultsLocalRecord, resultScore);
    assert.ok(resultsLocalRecord >= 1500);
